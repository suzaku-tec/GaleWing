...
class NewsSummaryControllerTest {
    ...
    void testDelete() {
        ...
        verify(newsSummaryRepository, times(1)).delete(anyString());
    }

    @Test
    void testAddSummary() {
        when(newsSummaryRepository.exists(anyString())).thenReturn(false);
        doNothing().when(newsSummaryRepository).insertSummary(anyString());

        newsSummaryController.addSummary(new ModelMock(), "uuid");
        verify(newsSummaryRepository).insertSummary(any());
    }

    @Test
    void testAddSummary_UuidNotExists() {
        when(newsSummaryRepository.exists(anyString())).thenReturn(true);

        newsSummaryController.addSummary(new ModelMock(), "uuid");
        verify(newsSummaryRepository, never()).insertSummary(any());
    }
