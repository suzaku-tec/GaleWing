...
class ViewsRepositoryTest {
  ...
  @Test
  void testGetViewFeedCount() {
    when(sqlManager.getResultList(any(), any())).thenReturn(List.of(new Site()));
    List<ViewFeedCount> result = viewsRepository.getViewFeedCount();
    Assertions.assertEquals(1, result.size());
  }

  @Test
  void testSelectViewSiteList() {
    when(sqlManager.getResultList(any(), any(), any())).thenReturn(List.of(new Site()));
    List<ViewSite> result = viewsRepository.selectViewSiteList("viewId");
    Assertions.assertEquals(1, result.size());
  }
