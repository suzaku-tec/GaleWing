...
export default class GaleWingApi {
  ...
  categoryFeed(categoryList: string[]): Promise<AxiosResponse<any>> {
    let baseUrl = new URL(window.location.href);
    let ajaxUrl = baseUrl.origin + this.apiUrls.categoryFeed;
    return axios.post(ajaxUrl, { categoryList: categoryList });
  }
