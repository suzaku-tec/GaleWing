...
export default class CardLayoutChgEvent implements IElementEvent {
  private static readonly CARD_ELEMENT_ID = 'cardLayout';
  ...
  execute(): void {
    ...
    var activeGridNavLink = navLinks.find(
      (element) =>
        ...
        (element as HTMLElement).dataset.layout === CardLayoutChgEvent.CARD_ELEMENT_ID,
    );
    ...
    contentLayouts.forEach((contentLayout) => {
      if (contentLayout.id === CardLayoutChgEvent.CARD_ELEMENT_ID) {
  ...
  private activate(navLinks: Element[]) {
    var gridNavLink = navLinks.find((element) => {
      return (element as HTMLElement).dataset.layout === CardLayoutChgEvent.CARD_ELEMENT_ID;
    });
